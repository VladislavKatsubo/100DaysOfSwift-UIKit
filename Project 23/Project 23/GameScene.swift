...
//  Project 23
//
//  Created by Vlad Katsubo on 6/9/22.
//
...
class GameScene: SKScene {
    ...
    func endGame(triggeredByBomb: Bool) {
        ...
        let scaleDown = SKAction.scale(by: 0.8, duration: 0.8)
//        let group = SKAction.group([scaleUp, scaleDown, scaleUp, scaleDown, scaleUp, scaleDown])
        let seq = SKAction.sequence([scaleUp, scaleDown, scaleUp, scaleDown, scaleUp, scaleDown])
        gameOver.run(seq)
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        if enemyType == 0 {
            ...
            if let path = Bundle.main.url(forResource: "sliceBombFuse.caf", withExtension: nil) {
            ...
            enemy = SKSpriteNode(imageNamed: "icecream")
            enemy.name = "extraEnemy"
        } else {
...
//        5 Give all enemies a circular physics body where the collisionBitMask is set to 0 so they don't collide.
        ...
        if enemyType == 7 {
            ...
            enemy.physicsBody?.velocity = CGVector(dx: 5 * 40, dy: 30 * 40)
        }
        ...
        if activeEnemies.count > 0 {
            for (index, node) in activeEnemies.enumerated().reversed() {
                if node.position.y < -140 {
                    ...
                    if node.name == "enemy" || node.name == "extraEnemy" {
                        node.name = ""
                        subtractLife()
